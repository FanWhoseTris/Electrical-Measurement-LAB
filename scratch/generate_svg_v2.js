...
for(let i=0; i<=10; i++) {
    // Non-linear mapping roughly: lower voltages take more space due to diode drop.
    // We will just do a simple linear for now, or slightly non-linear.
    // 0 -> 0, 10 -> 50. Wait, AC10V matches 0-50 on DC.
    let a = getRot(i*5, 0, 50); // Using DC scale for AC 10V for simplicity
    svg += `<g transform="rotate(${a})">
